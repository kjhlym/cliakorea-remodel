  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (imageFile) {
        ...
        imgFormData.append("files", imageFile);
      ...
      if (files.length > 0) {
        ...
        attachments = urls.map((url: string, idx: number) => ({
          name: files[idx].name,
          url: url
        }));
      }
      ...
      <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-8">
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          <div>
            ...
            <p className="text-xs text-gray-400 mb-3">자료에 대한 간략한 설명을 작성해주세요.</p>
            <textarea
              value={content}
              ...
              placeholder="자료의 주요 내용이나 활용 방법을 작성할 수 있습니다."
            />
          </div>
          ...
          <div>
            ...
            <div className="border-2 border-dashed border-gray-200 rounded-2xl p-10 text-center hover:bg-gray-50 transition-colors cursor-pointer relative group">
              ...
              <div className="flex flex-col items-center gap-3 text-gray-400 group-hover:text-blue-500 transition-colors">
                ...
                <div>
                  <p className="text-sm font-bold">파일을 클릭하거나 드래그하여 업로드</p>
                  <p className="text-xs mt-1">PDF, HWP, ZIP, 이미지 등 모든 파일 지원</p>
                </div>
              </div>
            </div>
